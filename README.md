...
p. 42: In the definition of a finite automaton, `F ⊂ S` should be
`F ⊆ S`: all states may be final.  In particular, the simplified
Empty construction described above has a single state that is both
initial and final, so `F = S = {i}`.
